...
const Financials = () => {
  ...
  const { convertInrToEth, loading: ethLoading } = useEthConversion();
          ...
          <CardContent>
            ...
            {ethLoading ? (
              <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                ...
                <span>Loading ETH...</span>
              </div>
            ) : (
              <p className="text-xs text-blue-600 dark:text-blue-400 font-mono mt-1">
                ≈ {convertInrToEth(revenue).toFixed(6)} ETH
              </p>
            )}
            <p className="text-xs text-muted-foreground mt-1">Sales + Recycling</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {ethLoading ? (
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 font-mono mt-1">
                ≈ {convertInrToEth(cost).toFixed(6)} ETH
              </p>
            )}
            <p className="text-xs text-muted-foreground mt-1">Payouts + Repairs</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {ethLoading ? (
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 font-mono mt-1">
                ≈ {convertInrToEth(profit).toFixed(6)} ETH
              </p>
            )}
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between py-2 border-b">
              <span className="text-muted-foreground">Recycling Costs:</span>
              <span className="font-medium">Processing & disposal</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-muted-foreground">Scrap Processing:</span>
              <span className="font-medium">Preparation for sale</span>
            </div>
          </div>
        </CardContent>
      </Card>
