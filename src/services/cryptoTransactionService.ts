 ...
 * Handles MetaMask transactions for Sepolia ETH
 */

import { ethers } from "ethers";
import { SEPOLIA_CONFIG } from "@/config/cryptoConfig";

// Global exchange rate: 1 ETH = 250,000 INR
 ...
 * Convert INR to ETH
 */
export function convertInrToEth(amountInr: number): number {
  if (amountInr <= 0) return 0;
  const eth = amountInr / EXCHANGE_RATE;
  // Round to 8 decimals (standard for ETH)
  return Math.round(eth * 100000000) / 100000000;
}

/**
 * Convert ETH to INR
 */
export function convertEthToInr(amountEth: number): number {
  if (amountEth <= 0) return 0;
  const inr = amountEth * EXCHANGE_RATE;
